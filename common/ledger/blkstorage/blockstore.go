...
import (
	...
	"github.com/npci/drunix/common/ledger/snapshot"
...
// RetrieveTxValidationCodeByTxID returns validation code and block number for the specified txID
...
func (store *BlockStore) AppendOrgBlockIndex(key []byte, orgVal []byte) error {
	return store.fileMgr.AppendOrgBlockIndex(key, orgVal)
}

func (store *BlockStore) AppendOrgMerkleInfo(key []byte, orgVal []byte) error {
	return store.fileMgr.AppendOrgMerkleInfo(key, orgVal)
}

func (store *BlockStore) SaveChannelHeadInfo(key []byte, orgVal []byte) error {
	return store.fileMgr.SaveChannelHeadInfo(key, orgVal)
}

func (store *BlockStore) GetOrgMetaValue(key []byte) ([]byte, error) {
	return store.fileMgr.GetOrgMetaValue(key)
}

func (store *BlockStore) AppendSavePoint(key []byte, orgVal []byte) error {
	return store.fileMgr.AppendSavePoint(key, orgVal)
}

func (store *BlockStore) GetSavePoint(key []byte) ([]byte, error) {
	return store.fileMgr.GetSavePoint(key)
}
