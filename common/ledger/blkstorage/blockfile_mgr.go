...
import (
	...
	"github.com/npci/drunix/internal/fileutil"
...
DRUNIX:
*/
func (mgr *blockfileMgr) AppendOrgBlockIndex(key []byte, orgVal []byte) error {
	if err := mgr.db.Put(key, orgVal, false); err != nil {
		return err
	}
	return nil
}

func (mgr *blockfileMgr) AppendOrgMerkleInfo(key []byte, orgVal []byte) error {
	if err := mgr.db.Put(key, orgVal, false); err != nil {
		return err
	}
	return nil
...
func (mgr *blockfileMgr) SaveChannelHeadInfo(key []byte, orgVal []byte) error {
	if err := mgr.db.Put(key, orgVal, false); err != nil {
		return err
	}
	return nil
...
func (mgr *blockfileMgr) GetOrgMetaValue(key []byte) ([]byte, error) {
	var b []byte
	var err error
	if b, err = mgr.db.Get(key); b == nil || err != nil {
		return nil, err
	}
	return b, nil
}

func (mgr *blockfileMgr) GetSavePoint(key []byte) ([]byte, error) {
	var b []byte
	var err error
	if b, err = mgr.db.Get(key); b == nil || err != nil {
		return nil, err
...
func (mgr *blockfileMgr) AppendSavePoint(key []byte, num []byte) error {
	if err := mgr.db.Put(key, num, false); err != nil {
		return err
	}
	return nil
}
