...
type ChainManager interface {
	GetChain(chainID, mspID string) Chain
}
...
func (h *Handler) deliverBlocks(ctx context.Context, srv *Server, envelope *cb.Envelope) (status cb.Status, err error) {
	...
	err = proto.Unmarshal(shdr.Creator, id)
	if err != nil {
		logger.Warningf("error parsing creator identity from %s: %s", addr, err)
		return cb.Status_BAD_REQUEST, nil
	}
	...
	if strings.Contains(strings.ToLower(orgId), "orderer") {
		orgId = ""
	} else {
		orgId = chdr.ChannelId
	}

	var chain Chain
	if chdr.IsLitepeer {
		logger.Warnf("utilising lpeer chain")
		chain = h.ChainManager.GetChain(chdr.ChannelId, channelconfig.LitePeerChainId)
		if chain == nil {
		...
		// now we are using channel-level split
		chain = h.ChainManager.GetChain(chdr.ChannelId, orgId)
		if chain == nil {
			// Note, we log this at DEBUG because SDKs will poll waiting for channels to be created
			// So we would expect our log to be somewhat flooded with these
			logger.Debugf("Rejecting deliver for %s because channel %s not found", addr, chdr.ChannelId)
