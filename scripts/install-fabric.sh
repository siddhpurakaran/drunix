...
#
# Has exactly the same functional power as bootstrap.sh
...
cloneSamplesRepo() {
    ...
    if [ -d test-network ]; then
        ...
        git clone -b main https://github.com/npci/drunix-samples.git && cd fabric-samples
    fi
...
pullBinaries() {
    ...
    download "${BINARY_FILE}" "https://github.com/npci/drunix/releases/download/v${VERSION}/${BINARY_FILE}"
    if [ $? -eq 22 ]; then
        echo
    ...
    download "${CA_BINARY_FILE}" "https://github.com/npci/drunix-ca/releases/download/v${CA_VERSION}/${CA_BINARY_FILE}"
    if [ $? -eq 22 ]; then
        echo
